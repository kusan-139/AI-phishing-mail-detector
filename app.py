...
def get_email_bodies(payload):
    ...
    This version correctly prioritizes the first HTML part it finds.
    """
    html_body = None
    text_body = None
    ...
    while parts_to_visit:
        ...
        if 'parts' in part:
            parts_to_visit.extend(part['parts'])
        ...
        if mime_type == 'text/html' and not html_body:
            try:
                html_body = base64.urlsafe_b64decode(body_data).decode('utf-8', 'ignore')
            except Exception:
                pass
        elif mime_type == 'text/plain' and not text_body:
            try:
                text_body = base64.urlsafe_b64decode(body_data).decode('utf-8', 'ignore')
            except Exception:
                pass
...
def calculate_phishing_score(sender, subject, plain_text_body, full_html_body):
    ...
    lower_body = plain_text_body.lower()
    lower_subject = subject.lower() if subject else ""
    urls = re.findall(r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', full_html_body)
...
def predict():
    ...
    if feedback_collection is not None:
        plain_text_body = BeautifulSoup(email_body, "html.parser").get_text()
        exact_match = feedback_collection.find_one({'body': plain_text_body})
        if exact_match:
    ...
    if not override_applied and model_exists:
        try:
            plain_text_body = BeautifulSoup(email_body, "html.parser").get_text()
            text_vector = feedback_vectorizer.transform([plain_text_body])
...
def process_email():
    ...
    try:
        ...
        html_body, text_body = get_email_bodies(payload)
        
        if text_body:
            analysis_body_text = text_body
        elif html_body:
            analysis_body_text = BeautifulSoup(html_body, "html.parser").get_text(separator=' ')
        else:
            analysis_body_text = ""
        
        render_body = html_body or text_body or ""

        parts = [payload]
        if 'parts' in payload:
            def find_all_parts(p):
                all_parts = [p]
                if 'parts' in p:
                    for sub_part in p['parts']:
                        all_parts.extend(find_all_parts(sub_part))
                return all_parts
            parts = find_all_parts(payload)
        ...
        return {"sender": sender, "subject": subject, "score": total_score, "prediction_text": result, "result_class": result_class, "reasons": reasons, "body": render_body}
    except Exception as e:
